...
export class PrismaProductRepository implements IProductRepository {
  ...
  async findById(id: string, tenantId: string): Promise<Result<Product, Error>> {
    ...
    if (!record) return Result.fail(new NotFoundError('Product', id));
    return Result.ok(this.toDomain(record));
  }

  async findBySku(sku: string, tenantId: string): Promise<Result<Product | null, Error>> {
    const record = await this.prisma.product.findFirst({ where: { sku, tenantId } });
  ...
  async findAll(
    ...
    return Result.ok(createPaginatedResult(records.map((r: any) => this.toDomain(r)), total, options));
  }
  ...
  async save(product: Product): Promise<Result<void, Error>> {
    ...
    await this.prisma.product.upsert({
      ...
      update: {
        ...
        costCents: data.costCents as any,
        currency: data.currency as string,
        stockQuantity: data.stockQuantity as number,
        stockMinAlert: data.stockMinAlert as any,
  ...
  async countByCategory(categoryId: string, tenantId: string): Promise<number> {
    return this.prisma.product.count({ where: { categoryId, tenantId } });
  }
  ...
  private toDomain(record: any): Product {
    return Product.reconstitute(record.id, {
      ...
      stockMinAlert: record.stockMinAlert ?? null,
