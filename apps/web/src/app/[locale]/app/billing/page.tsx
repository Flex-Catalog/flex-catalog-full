        ...
        <div className="p-6 space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            {status?.status === 'ACTIVE' || status?.status === 'TRIAL' ? (
              ...
              >
                ...
                {loading ? 'Abrindo portal...' : t('portal')}
              </button>
            ) : (
              <button
          ...
          {(status?.status === 'ACTIVE' || status?.status === 'TRIAL') && (
            <div className="bg-gray-50 rounded-lg px-4 py-3 text-xs text-gray-500 space-y-1">
              <p className="font-medium text-gray-700">Como cancelar sua assinatura:</p>
              <p>1. Clique em <strong>"Abrir Portal de Cobrança"</strong> acima</p>
              <p>2. Dentro do portal do Stripe, clique em <strong>"Cancelar plano"</strong></p>
              <p>3. O acesso permanece ativo até o fim do período pago</p>
            </div>
          )}
        </div>
      </div>
